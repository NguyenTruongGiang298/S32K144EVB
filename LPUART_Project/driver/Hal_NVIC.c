...
{
    uint8_t index = IRQn/32;
    return IP_NVIC->ISER[index] & (1U << (IRQn & 0x1F));
}
...
{
    uint8_t index = IRQn/32;
    return IP_NVIC->ISPR[index] & (1U << (IRQn & 0x1F));
}
...
{
    uint8_t index = IRQn/32;
    return IP_NVIC->IABR[index] & (1U << (IRQn & 0x1F));
}
void __NVIC_SetPriority(IRQn_Type IRQn,uint32_t priority)
{
    uint8_t index = IRQn / 4;
    uint8_t PRI_Nx = (IRQn % 4) * 8;
    IP_NVIC->IPR[index] &= ~(0xFF << PRI_Nx);
    IP_NVIC->IPR[index] |= ((priority << (8 - __NVIC_PRIO_BITS)) << PRI_Nx);
}
